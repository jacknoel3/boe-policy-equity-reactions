...
def split_samples(df: pd.DataFrame) -> dict[str, pd.DataFrame]:
    return {
        ...
        "covid_onward": df[df["date"] >= COVID_START_DATE].copy(),
    }
...
def save_tables(merged_df: pd.DataFrame, all_results: pd.DataFrame, all_magnitudes: pd.DataFrame) -> None:
    merged_path = TABLES_DIR / "merged_ftse250_v3.csv"
    merged_df.to_csv(merged_path, index=False)

    results_path = TABLES_DIR / "lp_results_ftse250_robustness_v3.csv"
    all_results.to_csv(results_path, index=False)

    magnitude_path = TABLES_DIR / "lp_magnitude_ftse250_robustness_v3.csv"
...
def plot_robustness_comparison(all_results: pd.DataFrame, shock: str, sample_name: str = "full") -> None:
    ...
    output_path = FIGURES_DIR / f"robustness_{shock}_{sample_name}_v3.png"
    fig.tight_layout()
...
def plot_subsample_comparison(
    ...
    output_path = FIGURES_DIR / f"subsamples{output_suffix}_{shock}_{robustness_spec}_v3.png"
    fig.tight_layout()
    fig.savefig(output_path, dpi=300)
    plt.close(fig)
    print(f"Saved figure: {output_path}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="FTSE 250 local projections with robustness checks.")
    parser.add_argument(
        "--samples",
        nargs="+",
...
def main() -> None:
    ...
    print("v3 FTSE 250 local projection robustness setup")
    ...
    save_tables(merged, all_results, all_magnitudes)
